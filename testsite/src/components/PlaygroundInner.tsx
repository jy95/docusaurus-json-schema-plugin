    ...
    >
      ...
      <Admonition type="tip">
        <p>
          You can use &nbsp;
          <Link
            to={`/playground?b64Schema=${Base64.encode(
              STRINGIFY_JSON(userSchema),
            )
              .replace("+", "-")
              .replace("/", "_")
              .replace("=", "")}`}
          >
            b64Schema query parameter
          </Link>
          &nbsp;with a JSON Schema in Base64URL format so that you can bookmark
          this page with wanted schema
        </p>
      </Admonition>
    </div>
  )
